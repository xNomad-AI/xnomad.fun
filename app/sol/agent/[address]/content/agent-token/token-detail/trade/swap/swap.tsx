...
import { appendInstruction, getTokenProgramId, prioritizationFee, serializeTransaction } from "./utils";
...
export function useSwap() {
    ...
    const createCalldata = async (params: OKXCallDataRequestParams, okx: OKXSwap, fee: number) => {
    ...
    const swap = useMemoizedFn(async (option: SwapOption) => {
        ...
        const [inputTokenCA, outputTokenCA] = type === "buy" ? [SOL_ADDRESS, tokenAddress] : [tokenAddress, SOL_ADDRESS];
        ...
        const tokenFeeAccount = getAssociatedTokenAddressSync(mint, owner, allowOwnerOffCurve, programId);
        const needFee = !inputProgramId.equals(TOKEN_2022_PROGRAM_ID) && !outProgramId.equals(TOKEN_2022_PROGRAM_ID);
        if (needFee) {
            try {
        ...
        try {
            const insForAgent = SystemProgram.transfer({
                ...
                lamports: +BigNumber(solAmount * FEE_FOR_AGENT).toFixed(0, BigNumber.ROUND_DOWN),
            });
            const insForTip = await provider.makeTransferInstruction(wallet.publicKey, tip);

            const ins = [...insForTip, insForAgent];
            if (needCreateFeeTokenAccount && needFee) {
                ins.unshift(createAssociatedTokenAccountInstruction(wallet.publicKey, tokenFeeAccount!, owner, mint, programId));
            }
            ...
            close();
            message(<BuySellSuccessfulToast isBuy={type === "buy"} status={status ? "success" : "failed"} txid={txid} />, {
                duration: 5000,
            });
        } catch (error: unknown) {
