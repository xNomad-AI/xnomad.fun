import { AddressLookupTableAccount, Connection, PublicKey, TransactionMessage } from "@solana/web3.js";
...
export function prioritizationFee(microLamports: number, computeUnitLimit: number) {
    const computeUnitPrice = Math.floor((microLamports * 100) / computeUnitLimit) * 10 ** 4;
...
export async function appendInstruction(tx: SwapTransaction, connection: Connection, ...instructions: TransactionInstruction[]) {
    if (tx instanceof VersionedTransaction) {
...
export async function getTokenProgramId(mintTokenAddress: string, connection: Connection) {
