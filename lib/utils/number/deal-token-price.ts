...
export const toSimpleNum = (num: number, precision = 2) => {
  ...
  const actualNum = Math.round(numFixed * Math.pow(10, precision)) / Math.pow(10, precision);
...
export const toThousandNum = (num: unknown, precision = 2, padZero = false) => {
  ...
  const isNegative = num < 0;
  const absNum = Math.abs(num);
  const numFixed = Math.round(absNum * Math.pow(10, precision)) / Math.pow(10, precision);
  const intNum = Math.floor(numFixed);
  const intNumStr = (intNum || 0).toString().replace(/(\d)(?=(?:\d{3})+$)/g, '$1,');
  const restStr = padZero
    ? (numFixed || 0).toFixed(precision).match(/\.(.*)$/g)
    : (numFixed || 0).toString().match(/\.(.*)$/g);
  return `${isNegative ? '-' : ''}${intNumStr}${restStr && precision !== 0 ? restStr[0] : ''}`;
};

export const toCardNum = (num: unknown, money = '', base = 1000, precision = 2, withNegative = false) => {
  let symbol = '';
  if (isValidNumber(num)) {
    if (num < 0) {
      symbol = '-';
...
export function isMinimumTokenNumber(num: number) {
  const targetNum = parseFloat(`0.${new Array(BASE_SMALL_NUM_PRECISION - 1).fill(0).join('')}1`);
...
export const dealCardTokenPrice = (
  ...
  const targetNum = parseFloat(`0.${new Array(smallNumPrecision - 1).fill(0).join('')}1`);
  let tokenString;
  if (!isValidNumber(num)) {
    tokenString = 'N/A';
  } else if (num === 0) {
    ...
    tokenString = deleteLastZero(
      toCardNum(num, '', base, Math.abs(num) >= precisionBase ? bigNumPrecision : smallNumPrecision).split('')
    );
  }
...
export const dealToolTipTokenPrice = (
  ...
  return dealCardTokenPrice(num, base, smallNumPrecision, bigNumPrecision, precisionBase);
};
